...
class DoxaCodeLensProvider : DaemonBoundCodeVisionProvider {
    ...
    override fun computeForEditor(editor: Editor, file: PsiFile): List<Pair<TextRange, CodeVisionEntry>> {
        ...
        try {
            ...
            for (l in lenses) {
                ...
                val offset = offsetFor(document.text, line, character)
                result.add(TextRange(offset, offset) to TextCodeVisionEntry(title, id, null, title, title, emptyList()))
            }
        } catch (_: Exception) {
        }
        return result
