...
void main() {
  group('Universe equality (strict, non-cumulative)', () {
    test('Type 0 converts with Type 0', () {
      expect(convTerms(const TType(0), const TType(0)), isA<ConvOk>());
    });

    test('Type 3 converts with Type 3', () {
      expect(convTerms(const TType(3), const TType(3)), isA<ConvOk>());
    });

    test('Type 0 does NOT convert with Type 1', () {
      final r = convTerms(const TType(0), const TType(1));
      expect(r, isA<ConvMismatch>());
      final m = r as ConvMismatch;
      expect((m.got as VType).level, 0);
      expect((m.expected as VType).level, 1);
    });

    test('Type 5 does NOT convert with Type 4 (either direction)', () {
      expect(convTerms(const TType(5), const TType(4)), isA<ConvMismatch>());
      expect(convTerms(const TType(4), const TType(5)), isA<ConvMismatch>());
    });
  });
  ...
  group('α-equivalence', () {
    test('two identity lambdas are α-equivalent', () {
      ...
      const id1 = TLam(TType(0), TBound(0));
      const id2 = TLam(TType(0), TBound(0));
      expect(convTerms(id1, id2), isA<ConvOk>());
    });

    test('constant function (λ_. c) with distinct constants differ', () {
      const f1 = TLam(TType(0), TType(1));
      const f2 = TLam(TType(0), TType(2));
  ...
  group('β-equivalence', () {
    test('(λx. x) y reduces to y before comparison', () {
      const reduced = TType(3);
      const lhs = TApp(TLam(TType(0), TBound(0)), TType(3));
      expect(convTerms(lhs, reduced), isA<ConvOk>());
    });

    test('(λx. λy. x) a b ≡ a', () {
      const app = TApp(
        TApp(TLam(TType(0), TLam(TType(0), TBound(1))), TType(5)),
        TType(7),
      );
      const a = TType(5);
  ...
  group('η-equivalence', () {
    test('(λx. f(x)) ≡ f when f is free', () {
      ...
      final lhsVal = eval(
        const TLam(TType(0), TApp(TBound(1), TBound(0))),
        envWithF,
      );
  ...
  group('Neutral spine comparison', () {
    test('same head, same single argument', () {
      const a = VNeutral(NApp(NVar(0), VType(1)));
      const b = VNeutral(NApp(NVar(0), VType(1)));
      expect(conv(1, a, b), isA<ConvOk>());
    });

    test('same head, different single argument → mismatch', () {
      const a = VNeutral(NApp(NVar(0), VType(1)));
      const b = VNeutral(NApp(NVar(0), VType(2)));
      final r = conv(1, a, b);
      expect(r, isA<ConvMismatch>());
      final m = r as ConvMismatch;
      expect((m.got as VType).level, 1);
      expect((m.expected as VType).level, 2);
    });

    test('different heads → mismatch', () {
      const a = VNeutral(NApp(NVar(0), VType(1)));
      const b = VNeutral(NApp(NVar(1), VType(1)));
    ...
    test('two-arg spines compare arg-by-arg; first differing arg reported', () {
      ...
      const a = VNeutral(NApp(NApp(NVar(0), VType(1)), VType(5)));
      const b = VNeutral(NApp(NApp(NVar(0), VType(1)), VType(6)));
      final r = conv(1, a, b);
      expect(r, isA<ConvMismatch>());
      final m = r as ConvMismatch;
      expect((m.got as VType).level, 5);
      expect((m.expected as VType).level, 6);
    });
    ...
    test('two-arg spines: first arg differs, second not reached', () {
      ...
      const a = VNeutral(NApp(NApp(NVar(0), VType(1)), VType(5)));
      const b = VNeutral(NApp(NApp(NVar(0), VType(9)), VType(5)));
      final r = conv(1, a, b);
      expect(r, isA<ConvMismatch>());
      final m = r as ConvMismatch;
      expect((m.got as VType).level, 1);
      expect((m.expected as VType).level, 9);
    });
  });
  ...
  group('Lambda domain annotations are NOT compared', () {
    test('(λ_: Type 0. Type 0) ≡ (λ_: Type 7. Type 0) since body matches', () {
      ...
      const a = TLam(TType(0), TType(0));
      const b = TLam(TType(7), TType(0));
      expect(convTerms(a, b), isA<ConvOk>());
    });
    ...
    test('differently-annotated identity functions are equivalent', () {
      ...
      const a = TLam(TType(0), TBound(0));
      const b = TLam(TType(1), TBound(0));
      expect(convTerms(a, b), isA<ConvOk>());
    });
  });
  ...
  group('Deep mismatch reporting', () {
    test('mismatch under two Pi layers points at the innermost diff', () {
      const a = TPi(TPi(TType(0), TType(0)), TType(0));
      const b = TPi(TPi(TType(0), TType(1)), TType(0));
    ...
    test('dependent Pi mismatch in codomain', () {
      // Codomains differ (TBound(0) vs TType(0)); opening at level 1 makes
