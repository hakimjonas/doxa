...
void main() {
  group('Term equality', () {
    test('same shape compares equal', () {
      expect(const TType(0), const TType(0));
      ...
      expect(const TLam(TType(0), TBound(0)), const TLam(TType(0), TBound(0)));
      expect(const TPi(TType(0), TBound(0)), const TPi(TType(0), TBound(0)));
    });

    test('different shape compares unequal', () {
      expect(const TType(0), isNot(const TType(1)));
      expect(const TBound(0), isNot(const TBound(1)));
      expect(const TFree('x'), isNot(const TFree('y')));
      expect(
        const TLam(TType(0), TBound(0)),
        isNot(const TPi(TType(0), TBound(0))),
      );
    });
  });
  ...
  group('openTerm', () {
    test('leaf types unchanged', () {
      expect(openTerm(const TType(0), 'x'), const TType(0));
    ...
    test('under one TLam, TBound(1) becomes TFree(x)', () {
      const input = TLam(TType(0), TBound(1));
      const expected = TLam(TType(0), TFree('x'));
      expect(openTerm(input, 'x'), expected);
    });

    test('under one TLam, TBound(0) stays bound', () {
      const input = TLam(TType(0), TBound(0));
    ...
    test('under nested TLam, TBound(2) becomes TFree(x)', () {
      ...
      const input = TLam(TType(0), TLam(TType(0), TBound(2)));
      const expected = TLam(TType(0), TLam(TType(0), TFree('x')));
      expect(openTerm(input, 'x'), expected);
    });

    test('under nested TLam, TBound(3) decrements to TBound(2)', () {
      // An even deeper outer reference should drop by one level.
      const input = TLam(TType(0), TLam(TType(0), TBound(3)));
      const expected = TLam(TType(0), TLam(TType(0), TBound(2)));
      expect(openTerm(input, 'x'), expected);
    });
  ...
  group('closeTerm', () {
    test('leaf types unchanged when name not present', () {
      expect(closeTerm(const TType(0), 'x'), const TType(0));
    ...
    test('under one TLam, TFree(x) becomes TBound(1)', () {
      const input = TLam(TType(0), TFree('x'));
      const expected = TLam(TType(0), TBound(1));
    ...
    test('under nested TLam, TFree(x) becomes TBound(2)', () {
      const input = TLam(TType(0), TLam(TType(0), TFree('x')));
      const expected = TLam(TType(0), TLam(TType(0), TBound(2)));
      expect(closeTerm(input, 'x'), expected);
    });
    ...
    test('different free variable name unaffected', () {
      const input = TLam(TType(0), TFree('y'));
      expect(closeTerm(input, 'x'), input);
    });
  });
  ...
  group('Round-trip: open(close(t, x), x) == t', () {
    ...
    final cases = <Term>[
      ...
      const TLam(TType(0), TFree('x')),
      const TLam(TType(0), TApp(TFree('x'), TBound(0))),
      const TPi(TFree('x'), TFree('x')),
      const TLam(
        TType(0),
        TLam(TType(0), TApp(TFree('x'), TApp(TBound(0), TBound(1)))),
      ),
    ];
