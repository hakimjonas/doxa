...
void main() {
  ...
  for (final entry in files.entries) {
    ...
    final sizeStr =
        bytes >= 1024 ? '${(bytes / 1024).toStringAsFixed(1)} KB' : '$bytes B';
    print(
      '${name.padRight(15)} | ${sizeStr.padLeft(8)} | ${best.toString().padLeft(9)} | ${median.toString().padLeft(11)} | $rate B/s',
    );
  }
  ...
  for (final line in lines.take(10)) {
    ...
    final display =
        lineSrc.length > 40 ? '${lineSrc.substring(0, 40)}...' : lineSrc;
    print(
      '${display.padRight(15)} | ${progUs.toString().padLeft(12)} | ${exprUs.toString().padLeft(12)}',
    );
  }
}
