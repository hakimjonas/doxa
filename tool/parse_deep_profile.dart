...
void main() {
  ...
  for (var i = 0; i < n; i++) {
    ...
    times.add(us);
    final declCount = r is Success<ParseError, SProgram>
        ? r.value.decls.length
        : (r is Partial<ParseError, SProgram> ? r.value.decls.length : '?');
    print('  ${(i+1).toString().padLeft(2)} | ${us.toString().padLeft(10)}  (${declCount} decls)');
  }
  times.sort();
  ...
  print('lexer fraction: ${(100.0*wsUs/best).toStringAsFixed(1)}% of total parse');
  print('');
  ...
  print('fixed overhead: likely $tinyUs us per file (parser init, trampoline setup)');
  ...
  for (final n in [1, 10, 50, 100, 200]) {
    ...
    print('  ${n.toString().padLeft(4)} decls | ${us.toString().padLeft(8)} us  (${perDecl.padLeft(6)} us/decl)');
  }
}
